import { fieldIsPrimary } from "../entity/fields";

import { getColumns, getRelColumns } from "../entity/columns";
...
export const fieldType = (field, create = true) =>
  `${field.isList ? "[" : ""}Update${field.type}Input${
    field.isList ? "]" : ""
  }`;
 ...
 * @return {string}
 */
export const fieldRequired = (field, create = true) =>
  isRequired(field, create) ? "!" : "";
...
${Object.keys(getColumns(types[name]))
    ...
    .map(
      fieldKey =>
        `    ${fieldKey}: ${types[name].fields[fieldKey].type}${fieldRequired(
          types[name].fields[fieldKey],
          true
        )}`
    )
    .join("\n")}
${Object.keys(getRelColumns(types[name]))
    .map(fieldKey =>
      createMutationInputField(fieldKey, types[name].fields[fieldKey])
    )
    .join("\n")}
}`;
...
${Object.keys(getRelColumns(types[name]))
    .map(fieldKey =>
      createMutationInputField(fieldKey, types[name].fields[fieldKey], false)
    )
    .join("\n")}
}`;
...
${Object.keys(getEntities(types))
    .map(typeKey => createMutationInputs(typeKey, types))
    .join("\n")}
${Object.keys(getEntities(types))
    .map(typeKey => updateMutationInputs(typeKey, types))
    .join("\n")}

type Mutation {
${Object.keys(getEntities(types))
    .map(typeKey =>
      createMutation(typeKey, /** @type {Type} */ (types[typeKey]))
    )
    .join("\n")}
}
`;
