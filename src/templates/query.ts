import { id, entities } from "../entity";
const { getIdKey, getId } = id;
...
export const createQueries = (name, entity) =>
  `
    ...
    ${lowerFirst(name)}(${getIdKey(entity.fields)}: ${
    getId(entity.fields).type
  }) : ${name}
`;
/**
...
${Object.keys(getEntities(entities))
    .map(entityKey => createQueries(entityKey, entities[entityKey]))
    .join("\n")}
}
`;
